...
class AmbientMixerSpider(scrapy.Spider):
    ...
    def parse_audio_listing(self, response: Response, **metadata):
        ...
        yield from response.follow_all(response.xpath("//div[@class='pages']//a"), callback=self.parse_audio_listing)
    ...
    def parse_audio_mix(self, response: Response, **metadata):
        title = response.css('div[id="home_description_top"]').css("h1::text").get().strip()
        description = response.css('div[id="home_description_audio_text"]').css("p::text").get().strip()
        # Parse breadcrumbs
        categories = [x.strip() for x in response.css('div[id="bread_crumbs"] a span::text').getall()]
        # get audio id for audio mix request
        audio_id = int(response.css('div[id="mixer_details"] .keep_left:last-child a').attrib['href'].split('/')[-1])
        yield Request(url=f'https://xml.ambient-mixer.com/audio-template?player=html5&id_template={audio_id}', callback=self.parse_audio_mix_xml,cb_kwargs={'title': title, 'description': description, 'categories': categories, 'audio_id': audio_id})
    ...
    def parse_audio_mix_xml(self, response: Response, **metadata):
        ...
        yield dict(mix={value['id_audio']: value for key, value in template_mix['audio_template'].items() if key.startswith('channel')}, **metadata)

    def start_requests(self):
        # self.task = self.progress_display.add_task(description="Downloading offers", total=self.max_scraped_offers)
        yield Request(url=self.base_url + 'most-rated-audio?page=1', callback=self.parse_audio_listing)
